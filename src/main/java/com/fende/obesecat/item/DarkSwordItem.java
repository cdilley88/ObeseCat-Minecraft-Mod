...
public class DarkSwordItem extends SkillSwordItem {
    public DarkSwordItem(Properties properties) { super(properties); }

    @Override protected int cooldownTicks() { return DarkSwordManager.COOLDOWN_TICKS; }
    ...
    @Override protected String captionKey() { return "item.obesecat.dark_sword.caption"; }
    @Override protected String effectKey() { return "item.obesecat.dark_sword.effect"; }
    ...
    protected boolean cast(ServerLevel level, Player player, InteractionHand hand, ItemStack stack) {
        ...
        if (entityTarget != null) {
            DarkSwordManager.schedule(level, player, entityTarget);
            return true;
        }
        BlockHitResult blockTarget = findBlockTarget(player);
        if (blockTarget != null) {
            DarkSwordManager.schedule(level, player, Vec3.atCenterOf(blockTarget.getBlockPos()));
            return true;
        }
        return false;
    }
    ...
    private static LivingEntity findEntityTarget(Player player) {
        ...
        Vec3 end = start.add(player.getViewVector(1.0F).scale(DarkSwordManager.RANGE));
        HitResult blockHit = player.pick(DarkSwordManager.RANGE, 1.0F, false);
        double maxDistance = blockHit.getType() == HitResult.Type.MISS
                ? DarkSwordManager.RANGE * DarkSwordManager.RANGE
    ...
    private static BlockHitResult findBlockTarget(Player player) {
        HitResult hit = player.pick(DarkSwordManager.RANGE, 1.0F, false);
